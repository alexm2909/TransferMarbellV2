...
console.log('🔎 Verificando variables de entorno para Netlify / preproducción...\n');
        ...
        res.on('end', () => {
          try {
            ...
            if (json.status === 'OK' || json.status === 'ZERO_RESULTS') {
              ...
              resolve({ ok: false, status: json.status, error: json.error_message });
            }
          } catch (err) {
...
(async () => {
  if (process.env.VITE_GOOGLE_MAPS_API_KEY) {
    ...
    if (res.ok) {
      ...
      console.log('   Asegúrate de habilitar Maps JavaScript API y Places API y de añadir restricciones de dominio si procede.');
    }
  } else {
  ...
  if (process.env.SMTP_TRANSPORT) {
    ...
    try {
      ...
      console.log('   Verifica que SMTP_TRANSPORT y FROM_EMAIL sean correctos y que tu proveedor permita conexiones desde Netlify/tu entorno.');
    }
  } else {
